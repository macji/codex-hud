import { emptyContext, emptyUsageLimits, type HudSnapshot, type RawStdinData, type UsageLimits } from '../types.js';
...
function modelName(model: RawStdinData['model']): string | undefined {
  if (typeof model === 'string') return model;
  if (model && typeof model === 'object') return model.display_name ?? model.name ?? model.id;
  return undefined;
}

function effortLevel(value: RawStdinData['effort']): string | undefined {
  if (typeof value === 'string') return value;
  if (value && typeof value === 'object' && typeof value.level === 'string') return value.level;
  return undefined;
}

function limits(raw: RawStdinData): UsageLimits | null {
...
export function snapshotFromStdin(raw: RawStdinData | null, cwd = process.cwd()): Partial<HudSnapshot> {
  if (!raw) return {};
