...
function officialContextWindow(model, windowSize) {
    if (isGpt55Model(model) && (windowSize === null || windowSize === undefined || windowSize < 400_000))
        return 400_000;
    return windowSize;
}
...
function readTranscriptContent(filePath) {
    try {
...
function currentTranscriptPath() {
...
function walkJsonlFiles(root, limit = 400) {
    const files = [];
    const stack = [root];
...
export function findCodexTranscript(cwd = process.cwd(), sessionId = currentSessionId(cwd), transcriptPath = currentTranscriptPath()) {
    if (transcriptPath)
        return transcriptPath;
    const currentOnly = process.env.CODEX_HUD_CURRENT_ONLY === '1';
    if (currentOnly && !sessionId)
        return null;
    ...
    for (const file of files.slice(0, 80)) {
        try {
            ...
            if (sessionId && head.includes(sessionId))
                return file;
            if (!currentOnly && head.includes(`"cwd":"${cwd.replaceAll('\\', '\\\\')}"`))
                return file;
        }
        catch {
            // Keep searching.
...
function normalizeTodoStatus(value) {
    ...
    if (value === 'in_progress' || value === 'running')
        return 'in_progress';
    if (value === 'pending' || value === 'not_started')
        return 'pending';
    return null;
}
function extractPlanTodos(payload) {
...
function applyTokenCount(snapshot, payload) {
...
function finishTool(tools, payload, timestamp) {
...
export function parseCodexTranscript(transcriptPath, options = {}) {
...
export function snapshotFromCodexTranscript(cwd = process.cwd(), options = {}) {
    const transcriptPath = findCodexTranscript(cwd);
    return transcriptPath ? parseCodexTranscript(transcriptPath, options) : {};
}
//# sourceMappingURL=codex-transcript-adapter.js.map