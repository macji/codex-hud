...
export function renderProjectLine(ctx) {
    ...
    if (ctx.config.display.showGit && snapshot.git) {
        ...
        if (ctx.config.display.gitMode === 'full' || ctx.config.display.gitMode === 'files') {
            if (snapshot.git.ahead)
                git += ` ↑${snapshot.git.ahead}`;
            if (snapshot.git.behind)
                git += ` ↓${snapshot.git.behind}`;
        }
        git += ')';
    ...
    if (ctx.config.display.showSessionName && snapshot.session.name)
        pieces.push(snapshot.session.name);
    return pieces.join(' │ ');
}
export function renderContextLine(ctx) {
    ...
    if (ctx.config.display.showContextBar && used !== null)
        parts.push(bar(used, 10, ctx.config));
...
export function renderUsageLine(ctx) {
    if (!ctx.config.display.showUsage || !ctx.snapshot.usage)
        return null;
    const l = labels(ctx);
    const five = ctx.snapshot.usage.fiveHour;
    const weekly = ctx.snapshot.usage.weekly;
    if (five.usedPercentage === null && weekly.usedPercentage === null)
        return null;
    const windows = [];
...
export function renderEnvironmentLine(ctx) {
    const l = labels(ctx);
    const parts = [];
    if (ctx.snapshot.model)
        parts.push(`${color.dim(l.model, ctx.config)} ${color.blue(ctx.snapshot.model, ctx.config)}${ctx.snapshot.reasoningEffort ? color.dim(`/${ctx.snapshot.reasoningEffort}`, ctx.config) : ''}`);
    if (ctx.config.display.showDuration && ctx.snapshot.session.duration)
        parts.push(`${color.dim(l.duration, ctx.config)} ${ctx.snapshot.session.duration}`);
    if (ctx.config.display.showCodexVersion && ctx.snapshot.codexVersion)
        parts.push(`${color.dim(l.codex, ctx.config)} ${ctx.snapshot.codexVersion}`);
...
export function renderToolsLine(ctx) {
    if (!ctx.config.display.showTools || ctx.snapshot.tools.length === 0)
        return null;
    const l = labels(ctx);
...
export function renderAgentsLine(ctx) {
    if (!ctx.config.display.showAgents || ctx.snapshot.agents.length === 0)
        return null;
    const l = labels(ctx);
...
export function renderTodosLine(ctx) {
    if (!ctx.config.display.showTodos || ctx.snapshot.todos.length === 0)
        return null;
    const l = labels(ctx);
