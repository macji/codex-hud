...
- npm。
- Codex CLI。
- 若要在 Codex TUI footer 中显示状态栏，需要 Codex CLI 支持原生 `[tui].status_line`。独立 `codex-hud` CLI 不依赖该能力。
...
`--setup` 会更新 `~/.codex/config.toml`，写入 Codex 原生 status line 项；如果已有配置，会先生成带时间戳的备份。
...
执行 setup 后请重启 Codex CLI，让 TUI 读取新的 status line。
...
- 确认 `~/.codex/config.toml` 中存在非空的 `[tui].status_line`。
- setup 后重启 Codex CLI。
...
- 确认重启的是重新构建后的 Codex。
- 确认当前 Codex build 支持原生 `[tui].status_line` 项。
- 如果 Codex 尚未拿到限额数据，rate-limit status line 项可能会省略或显示为未知。
